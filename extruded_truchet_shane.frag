...
float calcAO(in vec3 p, in vec3 n){
    ...
    float sca = 2., occ = 0.;
    for( int i = min(int(ubo.iTime * 60.0), 0); i<5; i++ ){
...
void main() {
    ...
    u = (u - ubo.iResolution.xy*.5)/ubo.iResolution.y;

    // Unit direction vector, camera origin and light position.
    vec3 r = normalize(vec3(u, 1)), o = vec3(0, ubo.iTime/2., -3), l = o + vec3(.25, .25, 2.);

    // Rotating the camera about the XY plane.
    r.yz = rot2(.15)*r.yz;
    r.xz = rot2(-cos(ubo.iTime*3.14159/32.)/8.)*r.xz;
    r.xy = rot2(sin(ubo.iTime*3.14159/32.)/8.)*r.xy;

    // Standard raymarching setup.
    float d, t = hash21(r.xy*57. + fract(ubo.iTime))*.5, glow = 0.;
    // Raymarch.
    for(int i=0; i<96; i++){
        ...
        //float rnd = hash21(r.xy + float(i)/113. + fract(ubo.iTime)) - .5;
