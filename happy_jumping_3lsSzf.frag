...
vec4 map( in vec3 pos, float atime )
{
    ...
    // eye
    {
    float blink = pow(0.5+0.5*sin(2.1*ubo.iTime),20.0);
...
void main() {
    ...
    for( int n=ZERO; n<AA; n++ )
    {
        ...
        vec2 p = (-ubo.iResolution.xy + 2.0*(fragCoord+o))/ubo.iResolution.y;
        // time coordinate (motion blurred, shutter=0.5)
        float d = 0.5*sin(fragCoord.x*147.0)*sin(fragCoord.y*131.0);
        float time = ubo.iTime - 0.5*(1.0/24.0)*(float(m*AA+n)+d)/float(AA*AA-1);
#else    
        vec2 p = (-ubo.iResolution.xy + 2.0*fragCoord)/ubo.iResolution.y;
