...
vec3 knot( in float t )
{
    ...
    float a1 = ubo.iTime*6.283185/10.0;
...
void main() {
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        vec2 p = (2.0*(fragCoord+o)-ubo.iResolution.xy)/ubo.iResolution.y;
        #else
        vec2 p = (2.0*fragCoord-ubo.iResolution.xy)/ubo.iResolution.y;
        #endif
