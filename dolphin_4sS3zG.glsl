...
vec2 sdWaterCheap( vec3 p )
{
    vec2 q = 0.1*p.xz;
	float f = 0.0;
    f += 0.50000*almostAbs(noise( q )); q = m2*q*2.02; q -= 0.1*ubo.iTime;
    f += 0.25000*almostAbs(noise( q )); q = m2*q*2.03; q += 0.2*ubo.iTime;
    f += 0.12500*almostAbs(noise( q )); q = m2*q*2.01; q -= 0.4*ubo.iTime;
    f += 0.06250*almostAbs(noise( q )); q = m2*q*2.02; q += 1.0*ubo.iTime;
...
vec3 calcNormalFish( in vec3 pos )
{
    ...
    vec3 n = vec3(0.0);
    for( int i=min(ubo.iFrame,0); i<4; i++ )
    {
...
void main()
{
	vec2 q = fragCoord.xy / ubo.iResolution.xy;
    vec2 p = -1.0 + 2.0 * q;
    p.x *= ubo.iResolution.x/ubo.iResolution.y;
    vec2 m = vec2(0.5);
	if( ubo.iMouse.z>0.0 ) m = ubo.iMouse.xy/ubo.iResolution.xy;

    //-----------------------------------------------------
    // animate
    //-----------------------------------------------------

	fishTime = 0.6 + 2.0*ubo.iTime - 20.0;
	...
	float an = 1.2 + 0.1*ubo.iTime - 12.0*(m.x-0.5);

	vec3 ta = vec3(fishPos.x,0.8,fishPos.z) - vec3(0.0,0.0,-2.0);
	vec3 ro = ta + vec3(4.0*sin(an),3.1,4.0*cos(an));

    // shake
	ro += 0.05*sin(4.0*ubo.iTime*vec3(1.1,1.2,1.3)+vec3(3.0,0.0,1.0) );
	ta += 0.05*sin(4.0*ubo.iTime*vec3(1.7,1.5,1.6)+vec3(1.0,2.0,1.0) );
	...
	// create view ray
	vec3 rd = normalize( p.x*uu + p.y*vv + 2.0*ww*(1.0+0.7*smoothstep(-0.4,0.4,sin(0.34*ubo.iTime))) );
	...
	if( tmat2.x>0.0 && (tmat1.x<0.0 || tmat2.x<tmat1.x) )
	{
        ...
        // caustics in dolphin
        float cc  = 0.65*texture( iChannel0, 2.5*0.02*posy.xz + 0.007*ubo.iTime*vec2( 1.0, 0.0) ).x;
        cc += 0.35*texture( iChannel0, 1.8*0.04*posy.xz + 0.011*ubo.iTime*vec2( 0.0, 1.0) ).x;
    ...
    // fade
	col *= smoothstep( 0.0, 1.0, ubo.iTime );
