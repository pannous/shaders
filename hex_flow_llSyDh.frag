...
void main(){
    ...
    float res = clamp(ubo.iResolution.y, 300., 600.); 
    
    // Aspect correct screen coordinates.
	vec2 u = (fragCoord - ubo.iResolution.xy*.5)/res;
    
    // Scaling and moving the screen coordinates.
    vec2 sc = u*4. + s.yx*ubo.iTime/8.;
    ...
    vec4 h = getHex(sc); // + s.yx*ubo.iTime/2.
    ...
    //float aRnd = sin(rnd*6.283 + ubo.iTime*1.5)*.5 + .5; // Animating the random number.
