...
void main(){
    ...
    vec3 ani = (abs(fract(ang/6.2831*aNum + ubo.iTime*dir*laneDir*.5) - .5)*2. - .66)/aNum0/2.;
    ang = max(arc + .025, ani);
    ...
    float resF = ubo.iResolution.y/450.;
    ...
    // Subtle vignette.
    uv = fragCoord/ubo.iResolution.xy;
