...
void main() {
    ...
    // Add gravitational lensing visual effects
    vec2 massPos = vec2(sin(t * 0.3) * 0.4, cos(t * 0.3) * 0.3);
    float distToMass = length(uv - massPos);

    // Einstein ring glow
    float ring = smoothstep(0.25, 0.15, abs(distToMass - 0.2));
    col += vec3(0.5, 0.7, 1.0) * ring * 0.8;

    // Accretion disk
    float disk = smoothstep(0.12, 0.05, abs(distToMass - 0.08));
    col += vec3(1.0, 0.6, 0.2) * disk * 1.5;

    // Black hole shadow
    float shadow = smoothstep(0.08, 0.05, distToMass);
    col *= (1.0 - shadow * 0.7);

    // Secondary mass effect
    vec2 mass2Pos = vec2(cos(t * 0.4) * 0.5, sin(t * 0.25) * 0.4);
    float dist2 = length(uv - mass2Pos);
    float ring2 = smoothstep(0.2, 0.12, abs(dist2 - 0.15));
    col += vec3(1.0, 0.5, 0.3) * ring2 * 0.6;
