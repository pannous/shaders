...
vec2 disp( in vec3 p )
{
	return vec2( pow( 0.5 + 0.5*cos( 1.0*ubo.iTime ), 2.0 ),
                 pow( 0.5 + 0.5*cos( 25.0*p.x  + 1.5*ubo.iTime)*
					            sin( 25.0*p.y  + 2.0*ubo.iTime )*
					            sin( 25.0*p.z  + 1.0*ubo.iTime ), 3.0) );
}
...
void main() {
    ...
    vec2 m = ubo.iMouse.xy/ubo.iResolution.xy;
	if( ubo.iMouse.z<0.0 ) m = vec2(0.0);
	
    // camera
	float an = -6.2*m.x + 0.2*sin(0.5*ubo.iTime) + 6.5;
