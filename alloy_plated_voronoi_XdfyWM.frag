...
vec2 hash22(vec2 p) {
    ...
    return p - .5;
    #else
    return cos(p*6.283 + ubo.iTime)*.5;
    //return abs(fract(p+ ubo.iTime*.25)-.5)*2. - .5; // Snooker.
    //return abs(cos(p*6.283 + ubo.iTime))*.5; // Bounce.
    #endif
...
float func2D(vec2 p){
    ...
    #ifdef RIGID_SCROLL
    p += vec2(-.2, 0)*ubo.iTime; // Scrolling.
    #endif
...
float func(vec2 p){
    ...
    vec2 e = vec2(8./ubo.iResolution.y, 0);
...
void main(){
    ...
    // Screen coordinates.
	vec2 u = (fragCoord.xy - ubo.iResolution.xy*.5)/ubo.iResolution.y;
    ...
    if(fi>3./12.) {
        ...
        //col *= vec3(1, .18, .28);
        //col = rotHue(col, mod(ubo.iTime/3. + 3.14159, 6.2831853) + (length(u*vec2(3., 5.))));        
        col *= .25;                   
    }
    else if(fi>2./12.) {
        
        col *= vec3(1, .18, .28);
        col = rotHue(col, mod(ubo.iTime/4., 6.2831853) + (length(u*vec2(2.5, 4.5))));// + ubo.iTime*.5
    }
    
    // Adding a bit of noise for a bit more authenticity.
    vec3 u3 = vec3(u, f); // Fake height, "ssd," according to function value.
    #ifdef RIGID_SCROLL
    u3.xy += vec2(-.2, 0)*ubo.iTime; // Scrolling.
    #endif
    ...
    //u = fragCoord/ubo.iResolution.xy;
